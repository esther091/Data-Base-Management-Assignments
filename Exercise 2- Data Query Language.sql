...
-- A query to select all columns from the products table
select * from products

-- A query to select only the name, price, and category columns from the products table.
select name, price, category from products

-- A query to find all products with a price greater than $50.
select * from products where price > 50

-- A query to find all products in the 'Electronics' category.
select * from products where category = 'Electronics'

-- A query to find all products that have a rating of 4.5 or higher.
select * from products where rating >= 4.5

-- A query to find all products that are in the 'Clothing' category AND have a price less than $30.
select * from products where category = 'Clothing' and price < 30

-- A query to find all products that are either in the 'Home' category OR have a stock quantity greater than 100.
select * from products where category = 'Home' or stock_quantity > 100

-- A query to find all products that are available AND NOT in the 'Toys' category.
select * from products where is_available = true and category <> 'Toys'

-- A query to find all products with a price between $20 and $100.
select * from products where price between 20 and 100
select * from products where price >= 20 and price <= 100

-- A query to list all products sorted by price in ascending order
select * from products order by price asc

-- A query to list all products sorted by name alphabetically.
select * from products order by name asc

-- A query to list all products sorted by rating in descending order, and then by price in ascending order when ratings are the same.
select * from products order by rating desc, price asc

-- A query to list the 5 most expensive products.
select * from products order by price desc limit 5

-- A query to list the 10 products with the highest stock quantity.
select * from products order by stock_quantity desc limit 10

--A query to display products sorted alphabetically by name, showing results 6-10
select * from products order by name asc limit 5 offset 5

--  A query to find all products whose name contains the word 'Premium'
select * from products where name like '%Premium%'

-- A query to find all products released in 2023.
select * from products where extract(year from release_date) = 2023

-- A query to find all products that have no rating 
select * from products where rating is null

-- A query to find the 3 most recently released products that are still available.
select * from products where is_available = true order by release_date desc limit 3;

-- A query to find all products in either the 'Books' or 'Stationery' categories with a price less than $25.
select * from products where category in ('Books', 'Stationery') and price < 25;
